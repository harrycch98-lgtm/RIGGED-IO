  ...
  if (req.method === 'POST' && req.url === '/api/lobby/create') {
    ...
    req.on('end', () => {
      ...
      const lobby = {
        ...
        players: [{ id: data.hostId, name: data.hostDisplayName || 'Host', host: true, isBot: false, ready: true, connected: true, lastSeen: now }],
  ...
  if (req.method === 'POST' && req.url === '/api/lobby/join') {
    ...
    req.on('end', () => {
      ...
      if (!lobby.players.some(player => player.id === data.playerId)) {
        lobby.players.push({ id: data.playerId, name: data.playerName, isBot: data.isBot === true, ready: data.isBot === true, connected: true, lastSeen: Date.now() });
      }
