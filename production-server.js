...
function handleHttp(req, res) {
  ...
  if (req.method === 'POST' && req.url === '/api/lobby/create') {
    ...
    req.on('end', () => {
      ...
      const lobby = {
        ...
        players: [{ id: data.hostId, name: data.hostDisplayName || 'Host', host: true, isBot: false, ready: true }],
  ...
  if (req.method === 'POST' && req.url === '/api/lobby/join') {
    ...
    req.on('end', () => {
      ...
      if (!lobby.players.some(player => player.id === data.playerId)) {
        lobby.players.push({ id: data.playerId, name: data.playerName, isBot: data.isBot === true, ready: data.isBot === true });
      }
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/game/state') {
    ...
    if (!lobby) return sendJson(res, 404, { error: 'Lobby not found' });
    sendJson(res, 200, { version: lobby.gameStateVersion || 0, state: lobby.gameState || null });
    return;
  }
...
function pruneAbandonedLobbies() {
  ...
  for (const [id, lobby] of lobbies) {
    ...
    if (abandonedSetup || expiredMatch) {
      lobbies.delete(id);
