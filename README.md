...
The frontend can now target a custom backend without editing source files.

Use query parameters on the hosted page:

```txt
?api=https://your-api-host:3000
```

If you also need to override the WebSocket endpoint used by the game client:

```txt
?api=https://your-api-host:3000&ws=wss://your-api-host:3000
```

Example:

```txt
https://harrycch98-lgtm.github.io/RIGGED-IO/?api=https://api.riggedio.com:3000&ws=wss://api.riggedio.com:3000
```

Supported override sources, in precedence order:

1. URL query params: `api`, `ws`
2. `window.RiggedConfig = { apiUrl, wsUrl }`
3. `localStorage` keys: `rigged.apiUrl`, `rigged.wsUrl`
4. Built-in local/production defaults
