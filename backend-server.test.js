...
(async () => {
  ...
  await request('/api/game/command', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ lobbyId: publicLobby.lobbyId, playerId: `guest-${suffix}`, command: { type: 'disrupt', args: [2] } }),
  });
  await request('/api/game/command', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ lobbyId: publicLobby.lobbyId, playerId: `guest-${suffix}`, command: { type: 'powerGrab', args: [4] } }),
  });
  ...
  assert.equal(drained.commands.length, 2, 'host should receive queued guest actions');
  assert.equal(drained.commands[0].command.type, 'disrupt');
  assert.equal(drained.commands[1].command.type, 'powerGrab');
