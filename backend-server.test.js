...
(async () => {
  ...
  const publishedState = await request('/api/game/state', {
    ...
    body: JSON.stringify({
      ...
      state: {
        ...
        latestAssassinationEvent: { id: 1, assassinId: 0, targetId: 1, stateIndex: 4 },
  ...
  assert.deepEqual(observedState.state.latestAssassinationEvent, { id: 1, assassinId: 0, targetId: 1, stateIndex: 4 }, 'guest should receive the assassination animation event');
