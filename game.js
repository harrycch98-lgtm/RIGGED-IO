(() => {
    ...
    lobbyUI.innerHTML = `
      ...
      <main class="lobby-entry-console">
        ...
        <section class="lobby-entry-actions">
          ...
          <label id="playerNameField" class="lobby-route lobby-name-route" for="playerNameInput">
            <span class="lobby-route-icon">@</span>
            <span><strong>Your Name</strong><input id="playerNameInput" type="text" maxlength="20" placeholder="Type your display name" autocomplete="nickname"></span>
          </label>
  ...
  function applyGameStateSnapshot(snapshot) {
    ...
    if (latestBroadcastEvent && Number(latestBroadcastEvent.id || 0) > lastPresentedBroadcastEventId) {
      ...
      presentBroadcast(Number(latestBroadcastEvent.channelIndex), String(latestBroadcastEvent.subtitle || ""));
    }
    updateUi(true);
  }
  ...
  function tickAction(player, dt) {
    ...
    if (player.action.type === "speech") {
      ...
      const siphonMult = 1;
      applyInfluenceGain(state, player.id, SPEECH_RATE * speechRateMult * player.speechBias * speechBoost * echoMult * modelPollingMult * hypeMult, dt, true, siphonMult);
      state.activePulse = 1;
    }
  ...
  function resolveDebateNight(debateId, stateIndex) {
    ...
    const gained = grantFlatStateInfluence(state, winner.id, DEBATE_WIN_BONUS);
  ...
  function completeDisruptOperation(mission) {
    ...
    targets.forEach((target) => {
      const protectedTreasury = hasTalent(target, "iron_curtain") && target.homeBase === state.index && target.mainBaseLevel >= 3;
      if (protectedTreasury) {
        results.push(target.name + " treasury protected");
      } else {
      ...
      if (target.homeBase === state.index) {
      ...
      if (state.police[target.id] && !policeBypassed) {
        if (target.cash >= POLICE_RIOT_BLOCK_COST) {
          target.cash -= POLICE_RIOT_BLOCK_COST;
          results.push(target.name + " police blocked office destruction");
          if (target.id === HUMAN) showToast("Police protected your District Office in " + state.abbr + " for " + formatMoney(POLICE_RIOT_BLOCK_COST) + ".");
          return;
        }
        state.police[target.id] = false;
      ...
      if (level <= 1) {
        ...
        results.push(target.name + " L1 office destroyed");
      } else {
        state.offices[target.id] = level - 1;
        results.push(target.name + " office reduced to L" + (level - 1));
      }
  ...
  function aiEarlyEconomyAction(player, rules, commit) {
    ...
    const upgradeTarget = offices
      .map((state) => {
        const level = officeLevel(state, player.id);
        const req = level < MINI_BASE_MAX_LEVEL ? miniBaseUpgradeReq(player, level + 1) : null;
    ...
    const influenceFarmState = offices
      ...
      .map((state) => {
        const req = miniBaseUpgradeReq(player, officeLevel(state, player.id) + 1);
  ...
  function tickAi(player, dt) {
    ...
    const guardedStates = states.filter((state) => state.police[player.id]);
    if (guardedStates.length && (player.cash < totalPoliceUpkeepDay(player) || projectedCashPerDay(player) < 0)) {
      const weakestGuard = guardedStates.sort((a, b) => officeLevel(a, player.id) - officeLevel(b, player.id) || a.ev - b.ev)[0];
      if (commit("police_removed", () => togglePolice(player.id, weakestGuard.index))) return;
    }
    ...
    const policeTargets = states
      .filter((state) => !state.police[player.id] && player.cash >= policeUpkeepDay(player, state) + rules.reserve)
      .map((state) => ({
        state,
        score: state.ev + officeLevel(state, player.id) * 5 + (player.homeBase === state.index ? 18 : 0)
          + (player.action?.type === "speech" && player.action.state === state.index ? 30 : 0),
      }))
      .sort((a, b) => b.score - a.score);
    if (guardedStates.length < rules.maxPolice && policeTargets.length && Math.random() < rules.police) {
      if (commit("police_deployed", () => togglePolice(player.id, policeTargets[0].state.index))) return;
    }

    if (canStartDisruptionOp(player, player.id)) {
      const disruptTargets = states
        .filter((state) => state.sabotageCooldown <= 0)
        .map((state) => {
    ...
    const upgradeTargets = states
      .map((state) => {
        const level = officeLevel(state, player.id);
        const req = level > 0 && level < MINI_BASE_MAX_LEVEL ? miniBaseUpgradeReq(player, level + 1) : null;
        if (!req || missions.some((mission) => mission.type === "officeUpgrade" && mission.player === player.id && mission.state === state.index)) return null;
    ...
    if (officeLvl > 0 && officeLvl < MINI_BASE_MAX_LEVEL) {
  ...
  function startAction(playerId, type, stateIndex) {
    ...
    if (type === "speech") {
      if (liveSpeakers.length >= 2) {
        if (playerId === HUMAN) showToast(`${state.abbr} already has an active two-leader debate.`);
        return false;
      }
    }
    ...
    const speechRateMult = executiveSpeech ? 2 : 1;
    ...
    if (type === "speech" && debateOpponent) {
      const debateId = "debate-" + Math.round(elapsed * 1000) + "-" + debateOpponent.id + "-" + player.id;
      [debateOpponent, player].forEach((candidate) => {
        candidate.action.debateId = debateId;
        candidate.action.debateRoll = Math.random() * 20;
        ...
        candidate.action.speechRateMult = SPEECH_SECONDS / DEBATE_SECONDS;
      });
      addAlert("DEBATE NIGHT: " + player.name + " challenged " + debateOpponent.name + " in " + state.name + ". Winner earns a +" + DEBATE_WIN_BONUS + "% influence surge.");
      broadcast(regionChannelIndex(state.region), "DEBATE NIGHT is live in " + state.name + ": " + debateOpponent.name + " versus " + player.name + ".");
      if (playerId === HUMAN) showToast("DEBATE NIGHT — beat " + debateOpponent.name + " for +" + DEBATE_WIN_BONUS + "% influence.");
    } else {
  ...
  function channelTakeoverCost(playerId, channel) {
    ...
    const procurementReclaim = owner && owner.id !== playerId && hasTalent(buyer, "procurement_office") && (channel.formerOwners || []).includes(playerId);
    if (procurementReclaim) return 5000;
  ...
  function broadcast(channelIndex, subtitle) {
    latestBroadcastEvent = { id: ++broadcastEventCounter, channelIndex, subtitle };
    lastPresentedBroadcastEventId = latestBroadcastEvent.id;
    presentBroadcast(channelIndex, subtitle);
  }

  function presentBroadcast(channelIndex, subtitle) {
  ...
  function showAssassinationBroadcast() {
    ...
    assassinationGif.src = `assassinate_broadcast_v2.gif?v=${Date.now()}`;
  ...
  function showPowerGrabBroadcast() {
    ...
    powerGrabGif.src = `power_grab_broadcast.gif?v=${Date.now()}`;
  ...
  function drawAllPoliceDeployments() {
    states.forEach((state) => {
      const guardedBy = players.filter((player) => state.police[player.id]);
      guardedBy.forEach((player, index) => {
        const spacing = 14;
        const x = state.cx + (index - (guardedBy.length - 1) / 2) * spacing;
        const y = state.cy - 20;
        drawPoliceShield(x, y, 13, player);
        if (policeAtRisk(player)) drawPoliceRiskWarning(x + 10, y - 10);
      });
    });
  }
  ...
  const TALENTS = {
    oligarchy: { name: "CORPORATE OLIGARCHY", sub: "CYBER-PLUTOCRATS", theme: "Fast capital, high-yield extraction, executive defense.", tiers: [
      ...
      { left:{id:"rapid_construction",name:"EXPRESS BUILDOUT",desc:"Main and District Office build or upgrade timers complete 33% faster.",live:true},
      ...
      { left:{id:"shadow_lobbying",name:"EXECUTIVE DIVIDENDS",desc:"Level 3 HQ income +50% and global state funding yield x1.25.",live:true,ult:true},
        right:{id:"executive_immunity",name:"RAPID-FIRE RALLIES",desc:"Speeches run at 2x speed for the same total influence. The speech action window is only half a campaign day.",live:true,ult:true} },
    ]},
    ...
    vanguard: { name: "IRON VANGUARD", sub: "CENTRAL AUTHORITY", theme: "Fortification, martial taxes, immediate retribution.", tiers: [
      ...
      { left:{id:"bureaucratic_hold",name:"CONSTRUCTION LOCKDOWN",desc:"DISRUPT freezes an upgrading enemy base for 2 campaign days.",live:true},
        right:{id:"checkpoint_grid",name:"DISRUPTION DELAY",desc:"Police-guarded states make enemy DISRUPT operations take 35% longer.",live:true} },
    ...
    futurist: { name: "CIVIC FUTURISTS", sub: "POLICY LAB", theme: "Scenario planning, policy cascades, resilient messaging.", tiers: [
      ...
      { left:{id:"fast_track_zoning",name:"OFFICE UPGRADE SAVINGS",desc:"District Office upgrades cost 15% less cash.",live:true},
        right:{id:"damage_control",name:"LOSS CONTAINMENT",desc:"If assassinated, nationwide influence loss drops from 5% to 3%.",live:true} },
    ...
    machine: { name: "CINDER MACHINE", sub: "STRIKE APPARATUS", theme: "Industrial discipline, coordinated disruption, hard territorial lockups.", tiers: [
      ...
      { left:{id:"assembly_line",name:"RAPID ASSEMBLY",desc:"District Office deploy and upgrade timers complete 40% faster.",live:true},
        right:{id:"red_tape_trap",name:"COOLDOWN CUT",desc:"Your DISRUPT operations leave states on 1 less day of cooldown.",live:true} },
    ...
    signal: { name: "TEAL WIRE ACCORD", sub: "SIGNAL CARTEL", theme: "Broadcast theft, interception, stealthy message control.", tiers: [
      ...
      { left:{id:"listening_posts",name:"GUARDED DONATIONS",desc:"Police-guarded states generate +10% influence cash.",live:true},
    ...
    ledger: { name: "IVORY LEDGER CLUB", sub: "BUDGET COMMITTEE", theme: "Quiet money, procedural control, closed-door efficiency.", tiers: [
      ...
      { left:{id:"budget_surplus",name:"HQ DIVIDENDS",desc:"Main Base passive cash output rises by 25% at every HQ level.",live:true,ult:true},
        right:{id:"procurement_office",name:"$5M CHANNEL BUYBACK",desc:"Taking back a news channel you previously owned costs a flat $5M.",live:true,ult:true} },
    ]},
  };
  ...
  function miniBaseUpgradeReq(player, nextLevel) {
    ...
    if (!req) return null;
    return {
      cash: discountedCost(req.cash * (hasTalent(player, "fast_track_zoning") ? 0.85 : 1) * (hasTalent(player, "system_overclock") ? 0.8 : 1), player),
  ...
  function policeUpkeepDay(player, state) {
    if (!player || !state) return 0;
    ...
    const officeCost = officeLevel(state, player.id) * POLICE_OFFICE_LEVEL_DAY;
    const rawCost = Math.max(POLICE_MIN_STATE_DAY, electoralCost + officeCost);
    return Math.round(rawCost * (hasTalent(player, "private_security") ? 0.5 : 1));
  }
  function totalPoliceUpkeepDay(player) {
    if (!player) return 0;
    return states.reduce((total, state) => total + (state.police[player.id] ? policeUpkeepDay(player, state) : 0), 0);
  }
  ...
  function financeBreakdown(player) {
    ...
    for (const st of states) {
      if (st.cashFreeze?.[player.id] > 0) {
        if (st.police[player.id]) police += policeUpkeepDay(player, st);
        continue;
      }
      const inf = st.influence[player.id];
      const level = officeLevel(st, player.id);
  ...
  function applyNationwideInfluencePenalty(player, amount) {
    ...
    states.forEach((state) => {
      ...
      state.influence[player.id] = clampInfluence(Math.max(floor, state.influence[player.id] - amount));
    });
  }
  ...
  function assassinate(playerId, stateIndex) {
    ...
    const threatenedSpeakers = debateId
      ...
      : [target];
    const policeDefenders = threatenedSpeakers.filter((candidate) => state.police[candidate.id]);
    ...
    policeDefenders.forEach((candidate) => {
      state.police[candidate.id] = false;
    ...
    const casualties = victims.map((victim) => {
      ...
      const targetStrip = hasTalent(victim, "damage_control") ? 3 : ASSASSINATE_STRIP;
      ...
      states.forEach((campaignState) => {
        ...
        campaignState.influence[victim.id] = clampInfluence(Math.max(floor, campaignState.influence[victim.id] - targetStrip));
      });
  ...
  const POLICE_RIOT_BLOCK_COST = 500;
  const POLICE_ASSASSINATION_BLOCK_COST = 5000;
  ...
  function disruptCostParts(player, targetOrTargets = null) {
    ...
    const discountedCashSiphon = discountedCost(cashSiphon, player);
    const discountedOfficeDamage = discountedCost(officeDamage, player);
    return {
  ...
  function disrupt(playerId, stateIndex) {
    ...
    if (state.sabotageCooldown > 0) {
      if (playerId === HUMAN) showToast(`${state.abbr} is on disruption cooldown (${Math.ceil(state.sabotageCooldown / CAMPAIGN_DAY_SECONDS)}d).`);
      return false;
    }
  ...
  function powerGrabCost(player, state) {
    ...
    return Math.round(discountedCost(POWER_GRAB_BASE_COST + Math.max(0, Number(state.ev) || 0) * POWER_GRAB_COST_PER_EV, player) * strikeFundDiscount);
  }

  function grantFlatStateInfluence(state, playerId, amount) {
    const current = adjustedInfluence(state, playerId);
    const gain = Math.min(amount, 100 - current);
    if (gain <= 0) return 0;
    let remaining = gain;
    ...
    for (const rival of rivals) {
      ...
      state.influence[rival.id] = clampInfluence(adjustedInfluence(state, rival.id) - taken);
      remaining -= taken;
    }
    const applied = gain - Math.max(0, remaining);
    state.influence[playerId] = clampInfluence(current + applied);
    return applied;
  }
  ...
  function powerGrab(playerId, stateIndex) {
    ...
    if (adjustedInfluence(state, playerId) >= 100) {
      if (playerId === HUMAN) showToast("You already have 100% influence in " + state.abbr + ".");
      return false;
    }
    player.cash -= cost;
    const grabAmount = hasTalent(player, "decentralized_hive") ? 30 : 20;
  ...
  function togglePolice(playerId, stateIndex) {
    ...
    addAlert(player.name + (state.police[playerId] ? " deployed police to " : " pulled police from ") + state.name + (state.police[playerId] ? " (" + formatMoney(policeUpkeepDay(player, state)) + "/day upkeep)." : "."));
    if (playerId === HUMAN) {
      showToast(state.police[playerId]
        ? "Police deployed: " + state.abbr + " guarded for " + formatMoney(policeUpkeepDay(player, state)) + "/day."
  ...
  function updateArmedTargetBanner(stateIndex = -1) {
    ...
    if (armedAction === "upgradeMiniBase") {
      banner.textContent = "\u25B6 UPGRADE ARMED \u2014 click one of your glowing District Offices (ESC to cancel)";
      return;
    }
    if (armedAction === "powerGrab") {
      const state = states[stateIndex];
      banner.textContent = state
        ? "\u25B6 POWER GRAB \u2014 " + state.abbr + " \u00B7 " + state.ev + " EV \u00B7 COST " + formatMoney(powerGrabCost(players[HUMAN], state)) + " \u2014 click to execute"
        : "\u25B6 POWER GRAB ARMED \u2014 hover a state to inspect its EV-based cost, then click to execute (ESC to cancel)";
      return;
    }
    if (armedAction === "togglePolice") {
      const state = states[stateIndex];
      banner.textContent = state
        ? "\u25B6 POLICE — " + state.abbr + " · " + formatMoney(policeUpkeepDay(players[HUMAN], state)) + "/DAY — click to deploy or withdraw"
        : "\u25B6 POLICE ARMED — hover a state to inspect daily upkeep, then click to deploy (ESC to cancel)";
      return;
    }
  ...
  const HOTBAR = [
    ...
    { key: "2", action: "publicSpeech", icon: "\u25C9", name: "SPEECH", cost: 0,
      tip: ["PUBLIC SPEECH", "Send your leader to rally here.", "Challenge a rival speech to start Debate Night.", "Debate winner gains a huge +15% state influence surge."] },
    ...
    { key: "4", action: "disrupt", icon: "\u26A0", name: "DISRUPT", cost: null,
      tip: ["STATE-WIDE DISRUPTION", "Steals $3M from every rival with an Office or HQ here.", "Lowers unprotected Offices and delays or cancels upgrades.", "No influence lead required. Base cost is $10M."] },
    { key: "5", action: "powerGrab", icon: "\u25C6", name: "POWER GRAB", cost: null,
      tip: ["POWER GRAB", "Instantly gain 20% influence, or 30% with Thirty-Point Power Grab.", "Price is $2M plus $1M per electoral vote.", "Power Grab Rebate reduces the final cost by 15%."] },
    { key: "6", action: "togglePolice", icon: "\u25EC", name: "POLICE", cost: 0,
      tip: ["DEPLOY POLICE", "Deploy independently in any state.", "Daily upkeep scales with electoral votes and Office level.", "Blocks assassination for $5M and DISRUPT destruction for $500K."] },
    { key: "7", action: "assassinate", icon: "\u2297", name: "ASSASSIN", cost: 40000,
      tip: ["ASSASSINATION", "Kill a rival leader during a speech.", "During Debate Night, one strike kills both leaders.", "A Debate Night double kill causes a triple -15% nationwide backlash."] },
  ];
  ...
  function buildHotbar() {
    ...
    hotbarEl.innerHTML =
      ...
      HOTBAR.map((s, i) =>
        ...
        '<span class="hcost">' + (s.cost === null ? "VARIES" : s.cost === 0 ? (s.action === "togglePolice" ? "BY STATE" : "FREE") : formatMoney(s.cost)) + '</span></button>'
      ).join("") + '</div></div>';
  ...
  function showHotTip(i, btn) {
    ...
    const costLabel = s.action === "upgradeMiniBase"
      ...
      ? `${formatMoney(policeUpkeepDay(human, states[selectedState]))}/D`
      : cost === 0
      ? "FREE"
      : formatMoney(cost);
    hotTipEl.innerHTML =
  ...
  function refreshHotbar() {
    ...
    hotbarEl.querySelectorAll(".hotslot").forEach((btn) => {
      ...
      const poor = s.action !== "powerGrab" && human && typeof cost === "number" && cost > 0 && human.cash < cost;
