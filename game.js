(() => {
  ...
  function startGame() {
    ...
    nextWorldEventAt = scheduleNextWorldEventDay(WORLD_EVENT_FIRST_DAY) * CAMPAIGN_DAY_SECONDS;
    alerts = [];
    missions = [];
    actionEffects = [];
    alertVersion = 0;
  ...
  function completeDisruptOperation(mission) {
    ...
    targets.forEach((target) => {
      ...
      if (worldEventActive("disrupt_siphon")) {
        ...
        if (siphoned > 0) {
          state.influence[target.id] = clampInfluence(adjustedInfluence(state, target.id) - siphoned);
          state.influence[player.id] = clampInfluence(adjustedInfluence(state, player.id) + siphoned);
  ...
  function scheduleNextWorldEventDay(fromDay = campaignDaysElapsed()) {
    const latest = Math.max(WORLD_EVENT_FIRST_DAY, currentMatchMode.days - WORLD_EVENT_LAST_DAY_BUFFER);
    const next = Math.max(WORLD_EVENT_FIRST_DAY, fromDay) + WORLD_EVENT_MIN_GAP_DAYS
  ...
  function tickWorldEvent(dt) {
    ...
    const day = campaignDaysElapsed();
    const tooEarly = day < WORLD_EVENT_FIRST_DAY;
  ...
  function powerGrab(playerId, stateIndex) {
    ...
    if (worldEventActive("reckless_power_grab")) {
      ...
      backlashStates.forEach((candidate) => {
        candidate.influence[playerId] = clampInfluence(Math.max(influenceFloor(player, candidate), adjustedInfluence(candidate, playerId) - 5));
