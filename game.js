(() => {
    ...
    hostScreen.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 600px; color: #34ff86; font-family: monospace; text-align: center;">
        ...
        <div style="border: 2px solid #34ff86; padding: 20px; margin-bottom: 30px; background: #0a0a0a; min-height: 100px;">
          ...
          <p style="color: #666; margin: 0; font-size: 13px;">Ready: ${readyCount}/${currentLobby.players.length}</p>
        </div>
  ...
  function showJoinedServerLobby() {
    ...
    const renderRoster = () => {
      const roster = document.getElementById('joinedLobbyRoster');
      if (!roster) return;
      const players = currentLobby.players || [];
      roster.innerHTML = `<strong>PLAYERS ${players.length}/${currentLobby.maxPlayers}</strong>` +
        (players.length ? players.map((player, index) => `<div style="margin-top:10px;color:#bfffe0">${index + 1}. ${escapeHtml(player.name || player.playerName || (index === 0 ? 'Host' : 'Player'))}${player.host || index === 0 ? ' (HOST)' : ''}</div>`).join('') : '<div style="margin-top:10px;color:#aaa">Connected to host lobby</div>');
    };
    renderRoster();
