(() => {
  ...
  function tickAi(player, dt) {
    ...
    if (speakingRival && player.cash >= assassinateCost(player, speakingRival) && Math.random() < 0.05) {
      const visibleSpeechStates = [speakingRival.action.state];
      if (Number.isInteger(speakingRival.action.decoyState)) visibleSpeechStates.push(speakingRival.action.decoyState);
      const assassinationState = visibleSpeechStates[Math.floor(Math.random() * visibleSpeechStates.length)];
      if (assassinate(player.id, assassinationState)) { player.aiDelay = AI_RULES[difficultyInput.value].delay + Math.random() * 2; return; }
    }
  ...
  function activeSpeechInState(stateIndex) {
    return players.find((candidate) =>
      ...
      (candidate.action.state === stateIndex || candidate.action.decoyState === stateIndex)
    ) || null;
  }

  function chooseSpeechDecoyState(realStateIndex) {
  ...
  function startAction(playerId, type, stateIndex) {
    ...
    const decoyState = type === "speech" && hasTalent(player, "skynet_protocol")
      ? chooseSpeechDecoyState(stateIndex)
      : null;
    player.action = { type, state: stateIndex, decoyState, left: times[type], total: times[type], vulnerableLeft, hypeBoost, speechRateMult };
    state.activePulse = 1;
  ...
  function updateUi(forceLog = false) {
    ...
    if (opponentTray) {
      opponentTray.innerHTML = players
        .map((player) => `
          <button class="opponent-chip${player.id === HUMAN ? " is-human" : ""}${player.locked > 0 ? " is-blackout" : ""}${isSpeaking(player) ? " is-speaking" : ""}" type="button" data-leader-player="${player.id}" aria-label="${player.id === HUMAN ? "Open your talent terminal" : `Inspect ${escapeHtml(player.name)} talent tree`}">
            ${leaderPortraitMarkup(player, "leader-portrait")}
  ...
  function drawActiveTacticalOverlays() {
    players.forEach((player) => {
      ...
      if (player.action.type === "speech") {
        ...
        if (Number.isInteger(player.action.decoyState)) {
          const decoyState = states[player.action.decoyState];
          if (decoyState) {
  ...
  const TALENTS = {
    oligarchy: { name: "CORPORATE OLIGARCHY", sub: "CYBER-PLUTOCRATS", theme: "Fast capital, high-yield extraction, executive defense.", tiers: [
      { left:{id:"aggressive_portfolio",name:"AGGRESSIVE PORTFOLIO",desc:"District Offices cost 20% less to deploy.",live:true},
        right:{id:"hostile_liquidation",name:"HOSTILE LIQUIDATION",desc:"DISRUPT siphons 50% extra capital from the rival treasury.",live:true} },
      { left:{id:"rapid_construction",name:"RAPID CONSTRUCTION",desc:"Main and District Office build or upgrade timers complete 33% faster.",live:true},
        right:{id:"private_security",name:"PRIVATE SECURITY",desc:"Police upkeep is cut 50%, making protection against enemy DISRUPT operations cheaper.",live:true} },
      { left:{id:"shadow_lobbying",name:"SHADOW LOBBYING",desc:"Level 3 HQ income +50% and global state funding yield x1.25.",live:true,ult:true},
        right:{id:"executive_immunity",name:"EXECUTIVE IMMUNITY",desc:"Speeches run at 2x speed for the same total influence. The speech action window is only half a campaign day.",live:true,ult:true} },
    ]},
    populist: { name: "POPULIST COALITION", sub: "NEURAL HIVE", theme: "Crowd mobilization, DISRUPT pressure, collective shielding.", tiers: [
      ...
      { left:{id:"general_strike",name:"GENERAL STRIKE",desc:"DISRUPT's office-damage component costs 50% less and resolves 25% faster.",live:true},
    ...
    syndicate: { name: "TECHNOCRATIC SYNDICATE", sub: "NETRUNNERS", theme: "Invisible operations, system siphoning, data redundancy.", tiers: [
      { left:{id:"system_overclock",name:"SYSTEM OVERCLOCK",desc:"HQ and District Office upgrades require 20% less cash and 20% less local influence.",live:true},
        right:{id:"signal_scrambler",name:"SIGNAL SCRAMBLER",desc:"DISRUPT's cash-siphon component bypasses enemy Police Force protection.",live:true} },
      { left:{id:"backdoor_exploits",name:"BACKDOOR EXPLOITS",desc:"DISRUPT cancels a target's District Office upgrade, forcing them to pay again.",live:true},
        right:{id:"ghost_servers",name:"GHOST SERVERS",desc:"Raises active DISRUPT operations from 1 to 3 and cuts the total DISRUPT cost by $2M.",live:true} },
      { left:{id:"skynet_protocol",name:"SKYNET PROTOCOL",desc:"Public speeches create a synchronized decoy speech in a random state. Assassinating the decoy wastes the full cost.",live:true,ult:true},
        right:{id:"blackout_bypass",name:"BLACKOUT BYPASS",desc:"Assassination blackout drops from 3 campaign days to 0.",live:true,ult:true} },
    ]},
    vanguard: { name: "IRON VANGUARD", sub: "CENTRAL AUTHORITY", theme: "Fortification, martial taxes, immediate retribution.", tiers: [
      { left:{id:"fortified_outposts",name:"FORTIFIED OUTPOSTS",desc:"Enemy DISRUPT office damage takes 100% longer against your District Offices.",live:true},
        right:{id:"martial_law_taxes",name:"MARTIAL LAW TAXES",desc:"Police-guarded bases generate +15% extra cash per day.",live:true} },
      { left:{id:"bureaucratic_hold",name:"BUREAUCRATIC HOLD",desc:"DISRUPT freezes an upgrading enemy base for 2 campaign days.",live:true},
        right:{id:"checkpoint_grid",name:"CHECKPOINT GRID",desc:"Police-guarded states make enemy DISRUPT operations take 35% longer.",live:true} },
    ...
    machine: { name: "CINDER MACHINE", sub: "STRIKE APPARATUS", theme: "Industrial discipline, coordinated disruption, hard territorial lockups.", tiers: [
      { left:{id:"picket_lines",name:"PICKET LINES",desc:"Rivals pay 25% more for DISRUPT's office-damage component against you.",live:true},
        right:{id:"wildcat_cells",name:"WILDCAT CELLS",desc:"DISRUPT's office-damage component resolves 35% faster.",live:true} },
      { left:{id:"assembly_line",name:"ASSEMBLY LINE",desc:"District Office deploy and upgrade timers complete 40% faster.",live:true},
        right:{id:"red_tape_trap",name:"RED TAPE TRAP",desc:"Your DISRUPT operations leave states on 1 less day of cooldown.",live:true} },
      { left:{id:"strike_fund",name:"STRIKE FUND",desc:"Successful DISRUPT operations refund 50% of their office-damage cost component.",live:true,ult:true},
        right:{id:"backlash_cells",name:"BACKLASH CELLS",desc:"If rivals DISRUPT your District Office, their cash flow from that state freezes for 3 days.",live:true,ult:true} },
    ]},
    signal: { name: "TEAL WIRE ACCORD", sub: "SIGNAL CARTEL", theme: "Broadcast theft, interception, stealthy message control.", tiers: [
      { left:{id:"dark_fiber",name:"DARK FIBER",desc:"DISRUPT's cash-siphon cost component is 25% lower.",live:true},
        right:{id:"signal_leak",name:"SIGNAL LEAK",desc:"Successful DISRUPT boosts your news channel influence 25% for 1 day.",live:true} },
  ...
  function assassinate(playerId, stateIndex) {
    ...
    const target = players.find((c) =>
      ...
      (c.action.state === stateIndex || c.action.decoyState === stateIndex)
    );
    ...
    player.cash -= cost;
    if (target.action.decoyState === stateIndex) {
      target.action.decoyState = null;
      state.activePulse = 1;
  ...
  function disruptCostParts(player, target = null) {
    const picketTax = target && hasTalent(target, "picket_lines") ? 1.25 : 1;
  ...
  function disrupt(playerId, stateIndex) {
    ...
    if (!target || target.id === playerId) {
      if (playerId === HUMAN) showToast("DISRUPT can only target the rival currently leading this state.");
      return false;
    }
    if (officeLevel(state, target.id) <= 0 && target.homeBase !== stateIndex) {
      if (playerId === HUMAN) showToast("The leading rival has no structure in this state to disrupt.");
      return false;
    }
    const costParts = disruptCostParts(player, target);
    ...
    const officeUpgradeMission = hasTalent(player, "backdoor_exploits")
      ? missions.find((mission) => mission.type === "officeUpgrade" && mission.state === stateIndex && mission.player === target.id)
      : null;
    ...
    const sabotageSeconds = guardedOperationTime(player, target, state, SABOTAGE_SECONDS);
    const seconds = Math.max(sabotageSeconds, riotTime(player, target, state));
    player.cash -= cost;
    missions.push({ type: "disrupt", player: playerId, target: target.id, state: stateIndex, backdoor, freeze, targetInfluence, siphonRate, cost: riotPortion, left: seconds, total: seconds });
    state.sabotageCooldown = Math.max(1, SABOTAGE_STATE_COOLDOWN_DAYS - (hasTalent(player, "red_tape_trap") ? 1 : 0)) * CAMPAIGN_DAY_SECONDS;
    state.activePulse = 1;
    addAlert(player.name + " launched a combined sabotage and riot operation against " + target.name + " in " + state.name + ".");
    triggerClickbait("INCITE_STRIKE", { player: playerId, target: target.id, state: stateIndex, stateName: state.name, factionName: player.name, opponentName: target.name });
    return true;
  }

  function powerGrabCost(player, state) {
    if (!state) return null;
    return discountedCost(POWER_GRAB_BASE_COST + Math.max(0, Number(state.ev) || 0) * POWER_GRAB_COST_PER_EV, player);
  }
  ...
  function powerGrab(playerId, stateIndex) {
    ...
    player.cash -= cost;
    const gained = grantFlatStateInfluence(state, playerId, 20);
    state.activePulse = 1;
  ...
  const HOTBAR = [
    ...
    { key: "4", action: "disrupt", icon: "\u26A0", name: "DISRUPT", cost: null,
      tip: ["COMBINED DISRUPTION", "Cash-siphon and office-damage effects land together.", "Steals rival cash, disrupts upgrades, and damages offices.", "Base cost is $10M; applicable talents can reduce it."] },
    { key: "5", action: "powerGrab", icon: "\u25C6", name: "POWER GRAB", cost: null,
      tip: ["POWER GRAB", "Instantly gain 20% influence in the selected state.", "Price is $2M plus $1M per electoral vote.", "High-value states cost substantially more."] },
    ...
    { key: "7", action: "assassinate", icon: "\u2297", name: "ASSASSIN", cost: 40000,
      tip: ["ASSASSINATION", "Kill a rival leader during a speech.", "Target gets a new leader and portrait.", "Target loses 5% influence and enters blackout."] },
  ];
  ...
  function hotbarCost(slot, human) {
    ...
    if (slot.action === "disrupt") {
      const state = states[selectedState];
      const leaderId = state ? leadingPlayer(state.index) : -1;
      const target = leaderId >= 0 ? players[leaderId] : null;
      return disruptCost(human, target);
    }
