(() => {
  ...
  function tickAi(player, dt) {
    ...
    if (guardedStates.length && (player.cash < 3000 || projectedCashPerDay(player) < 0)) {
    ...
    const ownedUnguarded = states
      .filter((state) => (player.homeBase === state.index || officeLevel(state, player.id) > 0) && !state.police[player.id])
      .map((state) => ({
        state,
        score: state.ev + officeLevel(state, player.id) * 5 + (leadingPlayer(state.index) !== player.id ? 12 : 0),
      }))
      .sort((a, b) => b.score - a.score);
    if (guardedStates.length < rules.maxPolice && ownedUnguarded.length && player.cash >= 5000 && Math.random() < rules.police) {
      if (commit("police_deployed", () => togglePolice(player.id, ownedUnguarded[0].state.index))) return;
    }
    ...
    if (canStartDisruptionOp(player, player.id)) {
      const disruptTargets = states
        ...
        .map((state) => {
          ...
          if (!target || target.id === player.id || (officeLevel(state, target.id) <= 0 && target.homeBase !== state.index)) return null;
          const cost = disruptCost(player, target);
          return {
            state,
            target,
            cost,
            score: state.ev * 1.5 + adjustedInfluence(state, target.id) * 0.35 + officeLevel(state, target.id) * 6 + (target.homeBase === state.index ? 10 : 0) + Math.random() * 6,
          };
        })
    ...
    const speechState = [states[preferredSpeechState], ...states]
      .filter(Boolean)
  ...
  function startAction(playerId, type, stateIndex) {
    ...
    const decoyStates = type === "speech" && hasTalent(player, "skynet_protocol")
  ...
  const TALENTS = {
    oligarchy: { name: "CORPORATE OLIGARCHY", sub: "CYBER-PLUTOCRATS", theme: "Fast capital, high-yield extraction, executive defense.", tiers: [
      ...
      { left:{id:"rapid_construction",name:"EXPRESS BUILDOUT",desc:"Main and District Office build or upgrade timers complete 33% faster.",live:true},
        right:{id:"private_security",name:"POLICE RETAINER",desc:"Police upkeep is cut 50%, making protection against enemy DISRUPT operations cheaper.",live:true} },
    ...
    syndicate: { name: "TECHNOCRATIC SYNDICATE", sub: "NETRUNNERS", theme: "Invisible operations, system siphoning, data redundancy.", tiers: [
      { left:{id:"system_overclock",name:"LEAN UPGRADES",desc:"HQ and District Office upgrades require 20% less cash and 20% less local influence.",live:true},
        right:{id:"signal_scrambler",name:"POLICE BYPASS",desc:"DISRUPT's cash-siphon component bypasses enemy Police Force protection.",live:true} },
  ...
  function assassinate(playerId, stateIndex) {
    ...
    if (hasTalent(target, "continuity_office")) {
      player.locked = Math.max(player.locked, 2 * CAMPAIGN_DAY_SECONDS);
      addAlert(target.name + "'s Assassin's Curfew forced " + player.name + " into a 2-day campaign blackout.");
    ...
    if (player.assassinationsToday > 1) {
      ...
      if (playerId === HUMAN) showToast("BACKLASH — another assassination today costs -5% influence nationwide.");
    }
    state.activePulse = 1;
    latestAssassinationEvent = {
  ...
  const HOTBAR = [
    ...
    { key: "7", action: "assassinate", icon: "\u2297", name: "ASSASSIN", cost: 40000,
      tip: ["ASSASSINATION", "Kill a rival leader during a speech.", "Target loses 5% influence and enters blackout.", "Every kill after your first that day costs you 5% nationwide."] },
  ];
