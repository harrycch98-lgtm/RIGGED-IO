(() => {
  ...
  function hitMainBase(point, ownerId = null) {
    for (let index = players.length - 1; index >= 0; index -= 1) {
      ...
      if (dx * dx + dy * dy <= 15 * 15) return { state: player.homeBase, player: player.id, x: pointOnMap.x, y: pointOnMap.y };
    }
    return null;
  }
    ...
    players.forEach((player) => {
      states.forEach((state) => {
        if (policeGuards(state, player.id, "hq") && player.homeBase === state.index) {
          const point = mainBasePoint(state);
          drawPoliceShield(point.x + 12, point.y - 10, 10, player);
          if (policeAtRisk(player)) drawPoliceRiskWarning(point.x + 20, point.y - 18);
        }
        if (policeGuards(state, player.id, "office") && officeLevel(state, player.id) > 0) {
          ...
          drawPoliceShield(point.x + 9, point.y - 9, 9, player);
  ...
  function policeUpkeepDay(player, state, building = "office") {
    ...
    return Math.round(rawCost * 0.7 * (hasTalent(player, "private_security") ? 0.5 : 1));
  }
  ...
  function playDebateBellSfx() {
    ...
    const now = ac.currentTime;
    const master = ac.createGain();
    master.gain.setValueAtTime(sfxLevel(0.9), now);
    master.connect(ac.destination);

    [0, 0.22].forEach((strikeDelay, strikeIndex) => {
      const strikeAt = now + strikeDelay;
      [510, 735, 1080, 1460].forEach((frequency, partialIndex) => {
        const oscillator = ac.createOscillator();
        const gain = ac.createGain();
        oscillator.type = partialIndex % 2 ? "square" : "sine";
        oscillator.frequency.setValueAtTime(frequency * (strikeIndex ? 1.015 : 1), strikeAt);
        gain.gain.setValueAtTime(0.0001, strikeAt);
        gain.gain.exponentialRampToValueAtTime(0.18 / (partialIndex + 1), strikeAt + 0.008);
        gain.gain.exponentialRampToValueAtTime(0.0001, strikeAt + 0.75 + partialIndex * 0.08);
        oscillator.connect(gain);
        gain.connect(master);
        oscillator.start(strikeAt);
        oscillator.stop(strikeAt + 1.1);
      });
    });
  }
  ...
  function updateArmedTargetBanner(stateIndex = -1) {
    ...
    if (armedAction === "powerGrab") {
      ...
      banner.textContent = state
        ? "\u25B6 POWER GRAB \u2014 " + state.abbr + " \u00B7 " + state.ev + " EV \u2014 click to execute"
    ...
    if (armedAction === "togglePolice") {
      const state = states[stateIndex];
      banner.textContent = state
        ? "\u25B6 POLICE — " + state.abbr + " — click to deploy or withdraw"
        : "\u25B6 POLICE ARMED — click a state to deploy or withdraw (ESC to cancel)";
      return;
    }
  ...
  const HOTBAR = [
    ...
    { key: "2", action: "publicSpeech", icon: "\u25C9", name: "SPEECH", cost: 0,
      tip: ["PUBLIC SPEECH", "Send your leader to rally here.", "Normal cooldown: 1 day. After Debate Night: 2 days.", "Challenge a rival speech to immediately replace both speeches with a debate."] },
    { key: "3", action: "upgradeMiniBase", icon: "\u25B3", name: "UPGRADE", cost: null,
      tip: ["UPGRADE DISTRICT OFFICE", "Improve one of your offices.", "Office level cannot exceed your HQ level.", "Click the office icon after arming.", "More cash, influence pressure, and defense."] },
    { key: "4", action: "disrupt", icon: "\u26A0", name: "DISRUPT", cost: null,
      tip: ["STATE-WIDE DISRUPTION", "Steals funds from every rival with an Office or HQ here.", "Lowers unprotected Offices and delays or cancels upgrades.", "No influence lead required."] },
    { key: "5", action: "powerGrab", icon: "\u25C6", name: "POWER GRAB", cost: null,
      tip: ["POWER GRAB", "Instantly gain 20% influence, or 30% with Thirty-Point Power Grab.", "The required reserves scale with electoral value.", "Power Grab Rebate reduces the requirement."] },
    { key: "6", action: "togglePolice", icon: "\u25EC", name: "POLICE", cost: 0,
      tip: ["DEPLOY POLICE", "Deploy independently in any state.", "Daily upkeep scales with electoral votes and Office level.", "Can block assassination and protect Offices from DISRUPT."] },
  ...
  function showHotTip(i, btn) {
    const s = HOTBAR[i];
    hotTipEl.innerHTML =
      '<div class="htt"><span>' + s.tip[0] + '</span></div>' +
