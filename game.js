(() => {
  ...
  function showHostingLobby() {
    ...
    const allReady = readyCount === currentLobby.players.length;
        ...
        confirmJoinBtn.onclick = async () => {
          ...
          try {
            ...
            if (lobby) {
              ...
              if (result) {
                statusDiv.innerText = 'Joined! Select your leader...';
                setTimeout(() => {
                  // Show party selection to the joiner
  ...
  function showWaitingScreenFull() {
    ...
    waitScreen.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 600px; color: #34ff86; font-family: monospace; text-align: center;">
        ...
        <div style="border: 2px solid #34ff86; padding: 30px; margin-bottom: 40px; background: #0a0a0a; min-height: 100px;">
          ...
          <p style="color: #aaa; margin: 0; font-size: 20px;">${currentLobby.players.length}/${currentLobby.maxPlayers} players</p>
    ...
    waitScreen.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 600px; color: #34ff86; font-family: monospace; text-align: center;">
        <h2 style="margin-bottom: 40px; font-size: 28px; color: #34ff86;">WAITING FOR HOST</h2>
        <p style="color: #aaa; margin-bottom: 20px;">Lobby Code: <strong style="color: #34ff86; font-size: 20px;">${currentLobby.inviteCode}</strong></p>
        <div style="border: 2px solid #34ff86; padding: 30px; margin-bottom: 40px; background: #0a0a0a; min-height: 100px;">
          <p style="color: #34ff86; margin: 0 0 15px 0;">Players In Lobby:</p>
          <p id="joinPlayerCount" style="color: #aaa; margin: 0; font-size: 20px;">${currentLobby.players.length}/${currentLobby.maxPlayers} players</p>
          <p style="color: #666; margin-top: 15px; font-size: 14px;">Waiting for host to start...</p>
        </div>
        <button id="leaveBtn" style="width: 100%; padding: 16px; background: transparent; color: #34ff86; border: 2px solid #34ff86; cursor: pointer; font-weight: bold; border-radius: 5px; font-size: 16px;">LEAVE LOBBY</button>
      </div>
    `;
  ...
  // Hide original multiplayer buttons - using new HOST/JOIN system instead
  if (createLobbyButton) createLobbyButton.style.display = 'none';
  if (copyInviteButton) copyInviteButton.style.display = 'none';
  ...
  // Old multiplayer buttons hidden - using new HOST/JOIN system instead
