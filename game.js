(() => {
  ...
  function selectBgmTrack() {
    ...
    const endThreshold = Math.max(3, currentMatchMode.days * 0.18);
    if (progress >= 0.86 || daysUntilElection() <= endThreshold * 0.5) return "end2";
    if (progress >= 0.72 || daysUntilElection() <= endThreshold) return "end1";
    if (progress >= 0.54) return "mid2";
    if (progress >= 0.36) return "mid1";
    return "early";
  }
  ...
  function hotbarActionAvailable(slot, human, state) {
    ...
    if (slot.action === "publicSpeech") {
      return !human.action && realSpeechesInState(state.index).length < 2;
    }
    if (slot.action === "upgradeMiniBase") {
    ...
    if (slot.action === "disrupt") {
      ...
      return state.sabotageCooldown <= 0 && canStartDisruptionOp(human, HUMAN) && targets.length > 0 && human.cash >= disruptCost(human, targets);
    }
    ...
    if (slot.action === "togglePolice") {
      return !!state.police[HUMAN] || human.cash >= policeUpkeepDay(human, state);
    }
  ...
  function hotbarCooldown(slot, human, state) {
    ...
    if (slot.action === "disrupt" && state?.sabotageCooldown > 0) {
      const days = Math.max(1, SABOTAGE_STATE_COOLDOWN_DAYS - (hasTalent(human, "red_tape_trap") ? 1 : 0));
      return { left: state.sabotageCooldown, total: days * CAMPAIGN_DAY_SECONDS };
    }
    return null;
  }
  ...
  function refreshHotbar() {
    ...
    hotbarEl.querySelectorAll(".hotslot").forEach((btn) => {
      ...
      if (cooldown) {
        ...
        if (cooldownLabel) cooldownLabel.textContent = campaignDaysLabel(cooldown.left);
