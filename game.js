(() => {
  ...
  function renderServerLobbyInMainMenu() {
    ...
    if (mainMenuAddBotButton) {
      ...
      mainMenuAddBotButton.disabled = !window.isServerLobbyHost || lobby.players.length >= lobby.maxPlayers;
    }
  }
  ...
  async function addBotToServerLobby() {
    if (!currentLobby?.id || !window.isServerLobbyHost) return false;
  ...
  function refreshMultiplayerUi() {
    ...
    if (currentLobby?.id && (window.isServerLobbyHost || window.isJoiner)) {
      ...
      if (mainMenuAddBotButton) {
        mainMenuAddBotButton.hidden = !window.isServerLobbyHost;
        mainMenuAddBotButton.disabled = !window.isServerLobbyHost || currentLobby.players.length >= currentLobby.maxPlayers;
      }
      if (createLobbyButton) createLobbyButton.textContent = window.isServerLobbyHost ? 'New Invitation' : 'Invite Friend';
      return;
    }
    pruneLobbyGuests();
    if (!multiplayerState.enabled) {
      ...
      if (mainMenuAddBotButton) {
        mainMenuAddBotButton.hidden = true;
        mainMenuAddBotButton.disabled = true;
      }
      renderLobbyParty();
      return;
  ...
  function loop(now) {
    ...
    if (!paused && !matchOver && !pipOpen) update(Math.min(0.5, dt));
    if (now - lastUi > 180) {
      updateUi();
      lastUi = now;
    }
