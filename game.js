    ...
    players.forEach((player) => {
      states.forEach((state) => {
        ...
        if (policeGuards(state, player.id, "office") && officeLevel(state, player.id) > 0) {
          ...
          drawPoliceShield(point.x, point.y, 12, player);
    ...
    if (armedAction === "togglePolice") {
      ...
      if (target && human) {
        ...
        banner.textContent = guarded
          ? "\u25B6 POLICE \u2014 " + state.abbr + " " + target.label + " \u00B7 GUARDED \u2014 click to withdraw"
          : "\u25B6 POLICE \u2014 " + state.abbr + " " + target.label + " \u00B7 UPKEEP " + formatMoney(policeUpkeepDay(human, state, target.building)) + "/DAY \u2014 click to deploy";
      } else {
        banner.textContent = "\u25B6 POLICE ARMED \u2014 hover your HQ or District Office to preview upkeep (ESC to cancel)";
      }
      return;
    }
