(() => {
  ...
  function tickFunding(player, dt) {
    ...
    if (hasTalent(player, "listening_posts")) {
      states.forEach((state) => {
        ...
        state.influence[player.id] = clampInfluence((state.influence[player.id] || 0)
  ...
  function miniBaseUpgradeReq(player, nextLevel) {
    ...
    if (!req) return null;
    return {
      cash: discountedCost(req.cash * (hasTalent(player, "rapid_construction") ? 0.7 : 1) * (hasTalent(player, "fast_track_zoning") ? 0.85 : 1) * (hasTalent(player, "system_overclock") ? 0.8 : 1), player),
  ...
  function updateArmedTargetBanner(stateIndex = -1) {
    ...
    if (armedAction === "togglePolice") {
      const state = states[stateIndex];
      banner.textContent = state
        ? "\u25B6 POLICE — " + state.abbr + " — click your HQ or District Office"
        : "\u25B6 POLICE ARMED — click your HQ or District Office icon (ESC to cancel)";
      return;
    }
  ...
  const HOTBAR = [
    ...
    { key: "6", action: "togglePolice", icon: "\u25EC", name: "POLICE", cost: 0,
      tip: ["DEPLOY POLICE", "Click one specific HQ or District Office.", "Police protect only the building they are deployed on.", "HQ police do not protect a District Office in the same state."] },
