(() => {
  ...
  function normalizeServerLobby(lobby, fallback = {}) {
    ...
    return {
      ...
      lobbyName: source.lobbyName || fallback.lobbyName || 'Untitled Lobby',
      isPublic: source.isPublic === true || source.visibility === 'public' || (source.isPublic == null && fallback.isPublic === true),
  ...
  async function createLobby(hostName, mode, difficulty, maxPlayers, lobbyName = 'Untitled Lobby', isPublic = false) {
    try {
      ...
      const res = await fetch(`${REST_BACKEND_URL}/api/lobby/create`, {
        method: 'POST',
  ...
  async function getPublicLobbies() {
    try {
      const res = await fetch(`${REST_BACKEND_URL}/api/lobbies?public=1`, { cache: 'no-store' });
  ...
  async function joinLobby(lobbyId, playerName) {
    try {
      const res = await fetch(`${REST_BACKEND_URL}/api/lobby/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          lobbyId: lobbyId,
    ...
    lobbyUI.innerHTML = `
      ...
      <main class="lobby-entry-console">
        ...
        <section class="lobby-entry-actions">
          ...
          <div id="lobbyContent" class="lobby-entry-content"></div>
  ...
  function refreshMultiplayerUi() {
    ...
    if (currentLobby?.id && (window.isServerLobbyHost || window.isJoiner)) {
      ...
      if (createLobbyButton) createLobbyButton.textContent = window.isServerLobbyHost ? 'New Lobby' : 'Host Lobby';
      return;
    }
    pruneLobbyGuests();
    if (!multiplayerState.enabled) {
    ...
    if (createLobbyButton) createLobbyButton.textContent = multiplayerState.host ? "New Lobby" : "Host Lobby";
    renderLobbyParty();
