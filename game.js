(() => {
  ...
  function renderLobbyLeaderStrip() {
    ...
    strip.innerHTML = slots.map((source, index) => {
      ...
      return `<article class="lobby-leader-slot ${stateClass}" style="--slot-color:${color}" title="${escapeHtml(partyName)}">${lobbyLeaderPortraitMarkup(player, factionIndex)}<span class="lobby-leader-party">${escapeHtml(partyName)}</span><span class="lobby-leader-name">${escapeHtml(name)}</span><span class="lobby-leader-state">${role}</span></article>`;
    }).join('');
  }
  ...
  async function createLobby(hostName, mode, difficulty, maxPlayers, lobbyName = 'Untitled Lobby', isPublic = false) {
    try {
      ...
      if (data.lobbyId) {
        ...
        await syncServerLobbyWithCrazyGames({ showInviteButton: true });
  ...
  async function joinLobby(lobbyId, playerName) {
    try {
      ...
      if (data.lobby) {
        ...
        await syncServerLobbyWithCrazyGames({ showInviteButton: false });
        scheduleServerLobbyPlayerUpdate();
        return data;
      }
      return null;
  ...
  async function addBotToServerLobby() {
    ...
    try {
      ...
      await syncServerLobbyWithCrazyGames({ showInviteButton: serverLobbyIsJoinable(currentLobby) });
