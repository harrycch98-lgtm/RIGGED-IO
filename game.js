(() => {
  ...
  function normalizeServerLobby(lobby, fallback = {}) {
    ...
    return {
      ...
      players: Array.isArray(source.players) ? source.players : (Array.isArray(fallback.players) ? fallback.players : []),
    };
  }
  ...
  function serverLobbyRosterHtml(lobby) {
    ...
    const rows = players.length
      ? players.map((player, index) => {
          ...
          const hostBadge = player.host || index === 0 ? ' <span style="color:#34ff86">(HOST)</span>' : '';
          return `<div style="margin-top:9px;color:#bfffe0">${index + 1}. ${escapeHtml(name)}${hostBadge}</div>`;
        }).join('')
  ...
  function renderServerLobbyInMainMenu() {
    ...
    if (multiplayerInvite) multiplayerInvite.textContent = `Invite code: ${normalizedInviteCode(currentLobby?.inviteCode)}`;
  ...
  function showLobbyInterface() {
    ...
    lobbyUI.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 500px; color: #34ff86; font-family: monospace; text-align: center;">
        ...
        <button id="hostBtn" style="width: 100%; padding: 20px; background: #34ff86; color: #000; border: 3px solid #34ff86; cursor: pointer; font-weight: bold; font-size: 18px; margin-bottom: 20px; border-radius: 5px;">INVITE FRIEND</button>
        ...
        <div id="lobbyContent" style="margin-top: 30px; text-align: left;"></div>
      </div>
    `;
  ...
  function showHostLobby() {
    ...
    content.innerHTML = `
      ...
      <button id="confirmHost" style="width: 100%; padding: 12px; background: #34ff86; color: #000; border: none; cursor: pointer; font-weight: bold; border-radius: 3px;">GENERATE INVITE CODE</button>
    ...
    setTimeout(() => {
      ...
      if (confirmBtn) {
        confirmBtn.onclick = () => {
          ...
          console.log('Host settings:', { mode, difficulty, maxPlayers });
  ...
  function showLobbyCode() {
    ...
    codeScreen.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 600px; color: #34ff86; font-family: monospace; text-align: center;">
        <h2 style="margin-bottom: 40px; font-size: 28px; color: #34ff86;">INVITE CODE</h2>
        ...
        <button id="changeHostLeaderBtn" style="width:100%;padding:16px;background:transparent;color:#34ff86;border:3px solid #34ff86;cursor:pointer;font-weight:bold;border-radius:5px;font-size:16px;margin-bottom:12px;">BACK TO PARTY / LEADER</button>
        ...
        <button id="startBtn" style="width: 100%; padding: 16px; background: #34ff86; color: #000; border: none; cursor: pointer; font-weight: bold; border-radius: 5px; font-size: 16px;">START GAME</button>
      </div>
    `;
  ...
  async function getUpdatedLobby(lobbyId) {
    try {
      const res = await fetch(`https://api.riggedio.com:3000/api/lobbies`, { cache: 'no-store' });
      const lobbies = await res.json();
  ...
  function showHostingLobby() {
    ...
    hostScreen.innerHTML = `
      <div style="background: #1a1a1a; border: 3px solid #34ff86; padding: 50px; border-radius: 15px; max-width: 600px; color: #34ff86; font-family: monospace; text-align: center;">
        <h2 style="margin-bottom: 30px; font-size: 28px; color: #34ff86;">HOSTING LOBBY</h2>
        ...
        <button id="hostChangeLeaderBtn" style="width:100%;padding:12px;background:transparent;color:#34ff86;border:2px solid #34ff86;cursor:pointer;font-weight:bold;border-radius:5px;margin-bottom:10px;">BACK TO PARTY / LEADER</button>
        ...
        <button id="backToCodeBtn" style="width: 100%; padding: 12px; background: transparent; color: #34ff86; border: 2px solid #34ff86; cursor: pointer; font-weight: bold; border-radius: 5px; margin-bottom: 10px;">BACK</button>
    ...
    setTimeout(() => {
      ...
      const hostChangeLeaderBtn = document.getElementById('hostChangeLeaderBtn');

      renderServerLobbyRoster('hostLobbyRoster');
      if (hostChangeLeaderBtn) hostChangeLeaderBtn.onclick = returnHostToLeaderSelection;
  ...
  function showJoinedServerLobby() {
    ...
    screen.innerHTML = `
      <div style="background:#1a1a1a;border:3px solid #34ff86;padding:42px;border-radius:15px;width:min(520px,calc(100vw - 48px));text-align:center;">
        <h2 style="margin:0 0 12px;">JOINED HOST LOBBY</h2>
  ...
  function refreshMultiplayerUi() {
    ...
    multiplayerInvite.textContent = `Invite code: ${multiplayerState.roomId.replace(/^rigged-/i, "")}`;
    if (copyInviteButton) copyInviteButton.disabled = !multiplayerState.inviteUrl;
    if (createLobbyButton) createLobbyButton.textContent = multiplayerState.host ? "New Invitation" : "Invite Friend";
